...
from . import admin
from ..models import ArticleType, Article, article_types, \
    Comment, User, Menu, ArticleTypeSetting, BlogInfo, Plugin
from .forms import SubmitArticlesForm, ManageArticlesForm, DeleteArticleForm, \
    ...
    AddBlogPluginForm, ChangePasswordForm, EditUserInfoForm
...
def submitArticles():
    ...
    # 改成form.validate_on_submit就出错,我也不知道为什么
    if form.title.data is not None and form.content.data is not None:
    ...
    return render_template('admin/submit_articles.html', form=form, title='发表博文', text=text)
...
def editArticles(id):
    ...
    return render_template('admin/submit_articles.html', form=form,title='编辑博文')
...
def manage_articles():
    ...
    form = ManageArticlesForm(types=types_id)
    ...
    if form.validate_on_submit():
        if form.types.data == -1:
            articles = Article.query.order_by(Article.create_time.desc()).all()
        else:
            articles = Article.query.filter_by(articleType_id=form.types.data).order_by(Article.create_time.desc()).all()
...
@admin.route('/manage-articles/delete-article/<int:id>', methods=['GET', 'POST'])
@login_required
def delete_article(id):
    blog=Article.query.filter_by(id=id).first()
    db.session.delete(blog)
    db.session.commit()
    flash("文章已删除",category='success')
...
@login_required
def manage_comments():
    admincomment = Comment.query.order_by(Comment.timestamp.desc()).filter_by(author_email='heiaoyang@foxmail.com').all()
    comment = Comment.query.order_by(Comment.timestamp.desc()).all()
    return render_template('admin/manage_comments.html', User=User,
                           admincomment=admincomment,comment=comment)
...
def custom_blog_info():
    user = User.query.filter_by(username='Mika').first()
    return render_template('admin/custom_blog_info.html', title='首页', user=user)
